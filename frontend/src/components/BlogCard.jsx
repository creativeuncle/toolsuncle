...
export default function BlogCard({ post }) {
  return (
    <article className="group overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 transition-all hover:-translate-y-1 hover:shadow-xl hover:shadow-slate-200/60 dark:hover:shadow-none dark:hover:border-slate-700">
      <Link to={`/blog/${post.slug}`} className="block aspect-video overflow-hidden bg-slate-100 dark:bg-slate-800">
        {post.image && (
          <img
            src={post.image}
            alt={post.title}
      ...
      <div className="p-5">
        {post.categorySlug ? (
          <Link
            ...
            className="inline-block rounded-full bg-indigo-50 dark:bg-indigo-500/10 px-2.5 py-1 text-xs font-medium text-indigo-600 dark:text-indigo-400 mb-3 hover:bg-indigo-100 dark:hover:bg-indigo-500/20"
          >
            {post.category}
          </Link>
        ) : (
        ...
        <Link to={`/blog/${post.slug}`}>
          <h3 className="text-base font-semibold mb-1.5 leading-snug hover:text-indigo-600 dark:hover:text-indigo-400">
            {post.title}
          </h3>
        </Link>
